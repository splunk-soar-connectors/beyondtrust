...
from beyondtrust_consts import (
    BEYONDTRUST_CONNECTIVITY_ENDPOINT,
    BEYONDTRUST_ERR_CONNECTIVITY_TEST,
    BEYONDTRUST_SUCC_CONNECTIVITY_TEST,
)
...
class BeyondtrustConnector(BaseConnector):
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = BEYONDTRUST_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(BEYONDTRUST_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(BEYONDTRUST_SUCC_CONNECTIVITY_TEST)
